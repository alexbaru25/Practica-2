...
Esta version limita el numero de coches que pasan seguidos, provocando que los peatones tengan que esperar menos tiempo
"""
import time
import random
...
class Monitor():
    ...
    def pass_north(self):
        if self.contador_peatones.value == NPED:
            return self.n_south_car.value == 0
        else:
            return self.n_ped.value == 0 and self.n_south_car.value == 0 and self.turno.value < Seguidos
    #Determina si los coches con direccion sur pueden pasar
    def pass_south(self):
        if self.contador_peatones.value == NPED:
            return self.n_north_car.value == 0
        else:
            return self.n_ped.value == 0 and self.n_north_car.value == 0 and self.turno.value < Seguidos
